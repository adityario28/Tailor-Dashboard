-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Worker assignment
-- ─────────────────────────────────────────────────────────────────────────────

-- 1. Tabel penjahit
CREATE TABLE trx.worker (
...
-- 2. Tambah kolom worker_id ke transaction
ALTER TABLE trx.transaction
ADD COLUMN worker_id INTEGER REFERENCES trx.worker(id);

-- 3. Extend trigger: ganti fungsi lengkap dengan blok worker_id tambahan
--    Menggunakan CREATE OR REPLACE sehingga trigger yang sudah terpasang
--    otomatis terupdate — tidak perlu DROP/CREATE TRIGGER ulang.
...
BEGIN
  ...
  -- Log worker_id changes — store worker name directly for self-contained audit history
  IF OLD.worker_id IS DISTINCT FROM NEW.worker_id THEN
    INSERT INTO trx.transaction_log (transaction_id, field_changed, old_value, new_value)
    VALUES (
      NEW.id,
      'worker_id',
      (SELECT name FROM trx.worker WHERE id = OLD.worker_id),
      (SELECT name FROM trx.worker WHERE id = NEW.worker_id)
    );
  END IF;
