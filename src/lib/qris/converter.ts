 ...
 * 5. Optionally insert Reference in Additional Data (tag 62)
...
export function convertQRIS(
  ...
  for (const el of elements) {
    ...
    // Insert reference in Additional Data Field (tag 62) after Merchant City (tag 60)
    if (el.tag === "60" && options.reference && !referenceInserted) {
      result.push(el);
      
      // Build Additional Data Field with Reference Label (sub-tag 05)
      const refValue = options.reference;
      const refTLV = `05${refValue.length.toString().padStart(2, "0")}${refValue}`;
      result.push(makeTLV("62", refTLV, "Additional Data Field Template"));
